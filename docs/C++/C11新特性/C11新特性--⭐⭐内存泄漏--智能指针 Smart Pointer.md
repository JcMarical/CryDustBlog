当你使用new时，你不用调用delete

栈上声明，堆上分配

[智能指针的C++实现 - 知乎 (zhihu.com)](https://zhuanlan.zhihu.com/p/64543967)
# 对象（栈作用域）生存期
...
* unique_ptr是作用域指针(scope pointer)
* 不能被复制
...
* 如果a指向b，b指向a。就会造成类似死锁的问题


## weak_ptr
* 弱指针，指向一个share_ptr管理的对象。
* 不会修改引用计数，只是用来观察
* 用lock可以转换为智能指针。
